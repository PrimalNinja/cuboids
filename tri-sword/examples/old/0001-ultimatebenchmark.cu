%%writefile ultimate_benchmark.cu
...
__global__ void nonagon_detect(
    ...
    // Extract edges (frequency band transitions)
    for (int i = 0; i < 9; i++) {
        int f1 = i * (FREQ_BINS / 9);
        int f2 = f1 + (FREQ_BINS / 9);
       
        float energy1 = 0.0f, energy2 = 0.0f;
        for (int f = f1; f < f1 + 5 && f < FREQ_BINS; f++) {
            energy1 += spectrogram[idx * TOTAL_POINTS + f];
        }
        for (int f = f2 - 5; f < f2 && f < FREQ_BINS; f++) {
            energy2 += spectrogram[idx * TOTAL_POINTS + FREQ_BINS + f];
        }
       
        // Ternary transition
        float diff = energy2 - energy1;
        nonagon.edges[i] = (diff > 1.0f) ? 1 : (diff < -1.0f) ? -1 : 0;
    }
   
    // Extract faces (time-frequency planes)
    for (int i = 0; i < 9; i++) {
        int f = i * (FREQ_BINS / 9);
        float energy = 0.0f;
       
        for (int t = 0; t < TIME_SLICES; t++) {
            for (int df = 0; df < FREQ_BINS / 9; df++) {
                energy += spectrogram[idx * TOTAL_POINTS + t * FREQ_BINS + f + df];
            }
        }
       
        nonagon.faces[i] = (energy > 2.0f) ? 1 : (energy > 0.5f) ? 0 : -1;
    }
   
    // Extract volume (3x3x3 spectral-temporal structure)
    int vol_idx = 0;
    for (int z = 0; z < 3; z++) { // Time
        for (int y = 0; y < 3; y++) { // Freq groups
            for (int x = 0; x < 3; x++) {
                int f_start = y * (FREQ_BINS / 3);
                int t = z;
               
                float energy = 0.0f;
                for (int f = f_start; f < f_start + (FREQ_BINS / 3) && f < FREQ_BINS; f++) {
                    energy += spectrogram[idx * TOTAL_POINTS + (t % TIME_SLICES) * FREQ_BINS + f];
                }
